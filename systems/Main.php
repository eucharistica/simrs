...
abstract class Main
{
    ...
    {
        // Hanya cookie, jangan SID di URL
        ini_set('session.use_only_cookies', '1');
        ini_set('session.use_trans_sid', '0');
    
        // Nama cookie unik per-app
        session_name('mlite_simrs');
    
        // Deteksi HTTPS (termasuk di balik proxy/tunnel)
        $isHttps = (
        ...
        $host   = $_SERVER['HTTP_HOST'] ?? 'simrs.rsudmatraman.my.id';
        $isIp   = filter_var(parse_url(($isHttps ? 'https://' : 'http://') . $host, PHP_URL_HOST), FILTER_VALIDATE_IP) !== false;
    
        // Jika host adalah IP, JANGAN set 'domain' (biarkan host-only). Browser akan abaikan 'domain' IP.
        $cookieParams = [
            'lifetime' => 0,
            'path'     => '/',
            // untuk IP: kosongkan domain -> host-only cookie
            'domain'   => $isIp ? '' : $host,
            'secure'   => $isHttps,   // HANYA true bila HTTPS
            'httponly' => true,
            'samesite' => 'Lax',
        ];
        ...
        if (PHP_VERSION_ID >= 70300) {
            ...
            // (Opsional) tambahkan SameSite untuk PHP lama
            header('Set-Cookie: ' . session_name() . '=deleted; path=/; ' . ($isHttps ? 'secure; ' : '') . 'HttpOnly; SameSite=Lax', false);
        }
    ...
    public static function verifyLicense($buffer)
    {
        ...
        $hasHeader = get_headers_list('X-Created-By') === 'Medic LITE Indonesia <rsudmatraman.jakarta.go.id>';
    ...
    public function getRegPeriksaInfo($field, $no_rawat)
    {
        ...
        ->select('pasien.no_peserta')
        ->select('pasien.tgl_lahir')
        ->select('pasien.no_ktp')
        ->select('pasien.agama')
    ...
    public function getRadiologiInfo($field, $no_rawat)
    {
        $row = $this->db('hasil_radiologi')
        ->where('no_rawat', $no_rawat)->oneArray();
        return $row[$field];
    }
    
    public function getPeriksaRadiologiInfo($field, $no_rawat)
    {
        $row = $this->db('periksa_radiologi')
        ->where('no_rawat', $no_rawat)->oneArray();
        return $row[$field];
    }

    public function getRawatInapInfo($field, $no_rawat)
    {
        $row = $this->db('reg_periksa')
        ->join('pasien', 'pasien.no_rkm_medis = reg_periksa.no_rkm_medis')
        ->where('no_rawat', $no_rawat)->oneArray();

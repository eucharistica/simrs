...
class Admin extends AdminModule
{
  ...
  private $_uploads = WEBAPPS_PATHX . '/berkasrawat/pages/upload';
  ...
  public function getManage()
  {
    $sub_modules = [
      ...
      ['name' => 'Pengajuan', 'url' => url([ADMIN, 'veronisa', 'pengajuan']), 'icon' => 'send', 'desc' => 'Pengajuan veronisa']
  ...
  public function anyIndex($page = 1)
  {
    ...
    if (isset($_POST['simpanberkas'])) {
      ...
      if(MULTI_APP) {
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => str_replace('webapps','',WEBAPPS_URLX).'api/berkasdigital',
        ...
        if($json['status'] == 'Success') {
          echo '<br><img src="'.WEBAPPS_URLX.'/berkasrawat/'.$json['msg'].'" width="150" />';
        } else {
    ...
    if (isset($_POST['deleteberkas'])) {
      if ($berkasPerawatan = $this->db('berkas_digital_perawatan')
        ...
        chdir('simrs/admin/'); //mengembalikan directory ke mlite/admin/
      }
    }
    ...
    FROM
      mlite_veronisa
    WHERE
      (mlite_veronisa.no_rkm_medis LIKE ? OR mlite_veronisa.no_rawat LIKE ? OR mlite_veronisa.nosep LIKE ?) 
      AND mlite_veronisa.tgl_registrasi BETWEEN '$start_date' AND '$end_date' ");
    ...
    WHERE
      (mlite_veronisa.no_rkm_medis LIKE ? OR mlite_veronisa.no_rawat LIKE ? OR mlite_veronisa.nosep LIKE ?) 
      AND mlite_veronisa.tgl_registrasi BETWEEN '$start_date' AND '$end_date' ORDER BY mlite_veronisa.no_rawat asc LIMIT $perpage OFFSET $offset");
  ...
  public function anyPengajuan($page = 1)
  {
    if (isset($_POST['submit'])) {
      if (!$this->db('mlite_veronisa')->where('nosep', $_POST['nosep'])->oneArray()) {
        $simpan_status = $this->db('mlite_veronisa')->save([
    ...
    $this->assign['searchUrl'] =  url([ADMIN, 'veronisa', 'pengajuan', $page . '?s=' . $phrase . '&start_date=' . $start_date . '&end_date=' . $end_date]);
    return $this->draw('pengajuan.html', ['veronisa' => $this->assign]);
  }
  ...
  public function getPDF($id)
  {
    ...
    $aturan_pakai = [];
    $list_obat = $this->db('detail_pemberian_obat')
      ->select('databarang.nama_brng')
      ->select('kodesatuan.satuan')
      ->select('detail_pemberian_obat.jml')
      ->select('aturan_pakai.aturan')
      ->join('databarang', 'detail_pemberian_obat.kode_brng=databarang.kode_brng')        
      ->join('kodesatuan','kodesatuan.kode_sat=databarang.kode_sat')
      ->leftJoin('aturan_pakai','aturan_pakai.no_rawat=detail_pemberian_obat.no_rawat AND detail_pemberian_obat.kode_brng=aturan_pakai.kode_brng')  
      ->where('detail_pemberian_obat.no_rawat', $no_rawat)
      ->toArray();
          
      foreach($list_obat as $data){
        $aturan = $this->db('detail_pemberian_obat')
        ->select('databarang.nama_brng')
        ->select('kodesatuan.satuan')
        ->select('detail_pemberian_obat.jml')
        ->select('aturan_pakai.aturan')
        ->join('databarang', 'detail_pemberian_obat.kode_brng=databarang.kode_brng')        
        ->join('kodesatuan','kodesatuan.kode_sat=databarang.kode_sat')
        ->join('aturan_pakai','aturan_pakai.no_rawat=detail_pemberian_obat.no_rawat')
        ->where('detail_pemberian_obat.no_rawat',$data['no_rawat'])
        // ->where('detail_pemberian_obat.kode_brng',$data['kode_brng'])
        // ->where('aturan_pakai.no_rawat',$data['no_rawat'])
        // ->where('aturan_pakai.kode_brng',$data['kode_brng'])
        ->oneArray();
        $aturan_pakai[] = $aturan;
      }
    ...
    */
    ...
    $this->tpl->set('aturan_pakai', $aturan_pakai);
    $this->tpl->set('list_obat', $list_obat);
    $this->tpl->set('resep_obat', $resep_obat);

...
class Site extends SiteModule
{
    ...
    public function routes()
    {
        ...
        $this->route('anjungan/operasi', 'getDisplayOperasi');
    ...
    public function getDisplayAPM()
    {
        ...
        $content = $this->draw('display.antrian.html', [
          'title' => $title,
          'logo' => $logo,
          'powered' => 'Powered by <a href="https://rsudmatraman.jakarta.go.id/">mLITE</a>',
          'username' => $username,
          'tanggal' => $tanggal,
    ...
    {
        $query = $this->db('mlite_antrian_loket')
    ...
    public function _resultDisplayJadwalOperasi()
    {
      ...
      $rows = $this->db('booking_operasi')
        ...
        ->select(['nm_pasien' => 'pasien.nm_pasien',
                  ...
                  'jam_mulai'    => 'booking_operasi.jam_mulai',
    ...
    public function getDisplayOperasi()
    {
      $logo  = $this->settings->get('settings.logo');
      $title = 'Display Operasi';
      $display = $this->_resultDisplayOperasi();

      $_username = $this->core->getUserInfo('fullname', null, true);
      $tanggal       = getDayIndonesia(date('Y-m-d')) . ', ' . dateIndonesia(date('Y-m-d'));
      $username      = !empty($_username) ? $_username : $this->core->getUserInfo('username');

      $content = $this->draw('display.operasi.html', [
        'logo' => $logo,
        'title' => $title,
        'powered' => 'Powered by <a href="https://rsudmatraman.jakarta.go.id/">mLITE</a>',
        'username' => $username,
    ...
    public function _resultDisplayOperasi()
    {
      $date = date('Y-m-d');
      $tentukan_hari = date('D', strtotime(date('Y-m-d')));
      $day = array(
      ...
      $rows = $this->db('laporan_operasi')
        ->join('operasi', 'operasi.no_rawat=laporan_operasi.no_rawat')
        ->join('dokter', 'dokter.kd_dokter=operasi.operator1')
        ->select([
                  'no_rawat'     => 'operasi.no_rawat',
                  'tanggal'      => 'laporan_operasi.tanggal_pa',
                  'kode'         => 'dokter.kd_dokter',
                  'namadok'      => 'dokter.nm_dokter',
                  'selesai'      => 'laporan_operasi.selesaioperasi',
                ])
        ->where('laporan_operasi.tanggal_pa', date('Y-m-d'))
        ->toArray();
    ...
    public function getSimpanNoRM()
    {
      ...
      if($query) {
          ...
          if($type == 'Apotek') {
            $reg_periksa = $this->db('reg_periksa')->where('tgl_registrasi', date('Y-m-d'))->where('kd_poli', '<>', 'IGDK')->where('no_rkm_medis', $no_rkm_medis)->oneArray();
            $this->db('resep_obat')
              ->where('tgl_peresepan', date('Y-m-d'))
              ->where('no_rawat', $reg_periksa['no_rawat'])
            ...
            $this->db('mlite_antrian_loket')->where('type', $type)->where('postdate', date('Y-m-d'))->where('no_rkm_medis', $no_rkm_medis)->update('status', 3);
          }
      } else {
          $res = [
              'status' => false,
    ...
    public function getAjax()
    {
      ...
      switch($show){
       default:
        ...
        case "tampilapotek":
          $result = $this->db('mlite_antrian_loket')->select('noantrian')->where('type', 'Apotek')->where('postdate', date('Y-m-d'))->desc('start_time')->oneArray();
          $noantrian = '';
          if($result) {
            $noantrian = $result['noantrian'];
          }
    ...
    public function getDisplayBed()
    {
        ...
        $logo  = $this->settings->get('settings.logo');
        $displaykhusus = $this->_resultDisplayBedKhusus();
        $displayperina = $this->_resultDisplayBedPerina();
        $displaydewasa1 = $this->_resultDisplayBedDewasa1();
        $displaydewasa3 = $this->_resultDisplayBedDewasa3();
        $displaydewasaiso = $this->_resultDisplayBedDewasaIso();
        $displayanak1 = $this->_resultDisplayBedAnak1();
        $displayanak2 = $this->_resultDisplayBedAnak2();
        $displayanak3 = $this->_resultDisplayBedAnak3();
        $displayanakiso = $this->_resultDisplayBedAnakIso();
        $displayVK = $this->_resultDisplayBedVK();
        ...
        $content = $this->draw('display.bed.html', [
          ...
          'tanggal' => $tanggal,
          'running_text' => $this->settings->get('anjungan.text_poli'),
          'displaykhusus' => $displaykhusus,
          'displayperina' => $displayperina,
          'displaydewasa1' => $displaydewasa1,
          'displaydewasa3' => $displaydewasa3,
          'displaydewasaiso' => $displaydewasaiso,
          'displayanak1' => $displayanak1,
          'displayanak2' => $displayanak2,
          'displayanak3' => $displayanak3,
          'displayanakiso' => $displayanakiso,
          'displayVK' => $displayVK,
        ]);
    ...
    public function _resultDisplayBedKhusus()
    {

        $query = $this->db()->pdo()->prepare("SELECT a.nm_bangsal, b.kelas , a.kd_bangsal FROM bangsal a, kamar b WHERE a.kd_bangsal = b.kd_bangsal AND b.statusdata = '1' AND b.kd_bangsal in ('B0002','B0063','B0082') GROUP BY b.kd_bangsal, b.kelas");
        $query->execute();
        $rows = $query->fetchAll(\PDO::FETCH_ASSOC);;
    ...
    public function _resultDisplayBedPerina()
    {

        $query = $this->db()->pdo()->prepare("SELECT a.nm_bangsal, b.kelas , a.kd_bangsal FROM bangsal a, kamar b WHERE a.kd_bangsal = b.kd_bangsal AND b.statusdata = '1' AND b.kd_bangsal in ('B0006') GROUP BY b.kd_bangsal, b.kelas");
        $query->execute();
        $rows = $query->fetchAll(\PDO::FETCH_ASSOC);;
    ...
    public function _resultDisplayBedDewasa1()
    {
        ...
        $result = [];
                $row['kosong'] = $this->db('kamar')
                  ->select(['jumlah' => 'COUNT(kamar.status)'])
                  ->join('bangsal', 'bangsal.kd_bangsal = kamar.kd_bangsal')
                  ->where ('bangsal.kd_bangsal','B0011')
                  ->where('kamar.status','KOSONG')
                  ->where('kamar.statusdata','1')
                  ->oneArray();
                if(empty($row['kosong']['jumlah'])) {
    ...
    public function _resultDisplayBedDewasa3()
    {
            ...
            $result = [];
                $row['kosong'] = $this->db('kamar')
                  ...
                  ->where('kamar.status','KOSONG')
                  ->where('kamar.statusdata','1')
                  ->in ('bangsal.kd_bangsal',['B0005','B0007','B0060'])
                  ->oneArray();
                if(empty($row['kosong']['jumlah'])) {
                  $row['kosong']['jumlah'] = 0;
                }
    ...
    public function _resultDisplayBedAnak1()
    {
    ...
    public function _resultDisplayBedAnak2()
    {
    ...
    public function _resultDisplayBedAnak3()
    {
    ...
    public function _resultDisplayBedDewasaIso()
    {
    ...
    public function _resultDisplayBedAnakIso()
    {
    ...
    public function _resultDisplayBedVK()
    {
            ...
            $result = [];
                $row['kosong'] = $this->db('kamar')
                  ...
                  ->where('kamar.status','KOSONG')
                  ->where('kamar.statusdata','1')
                  ->in ('bangsal.kd_bangsal',['B0004','B0019','B0058'])
                  ->oneArray();
                if(empty($row['kosong']['jumlah'])) {
                  $row['kosong']['jumlah'] = 0;
                }
    ...
    public function getSepMandiriBikin()
    {
      ...
      if(!$this->db('reg_periksa')->where('no_rkm_medis', $slug[4])->where('tgl_registrasi', $date)->oneArray()) {
        $tentukan_hari=date('D',strtotime(date('Y-m-d')));
        $day = array(
          'Sun' => 'AKHAD',
          'Mon' => 'SENIN',
        ...
        $maping_poli_bpjs = $this->db('maping_poli_bpjs')->where('kd_poli_bpjs', $slug[5])->oneArray();
        $jadwal = $this->db('jadwal')->where('hari_kerja', $hari)->where('kd_poli', $maping_poli_bpjs['kd_poli_rs'])->oneArray();
        $poliklinik = $this->db('poliklinik')->where('kd_poli', $jadwal['kd_poli'])->oneArray();
    ...
    public function postSaveSEP()
    {
        ...
        if($this->settings->get('jkn_mobile.kirimantrian') == 'ya') {
          ...
          $pasien = $this->db('pasien')->where('no_rkm_medis', $_POST['nomr'])->oneArray();
          $reg_periksa = $this->db('reg_periksa')->where('tgl_registrasi', date('Y-m-d'))->where('no_rkm_medis', $_POST['nomr'])->oneArray();
    ...
    public function getDisplayCheckin($referensi)
    {
      ...
      //echo 'Checkin';
      $referensi = $this->db('mlite_antrian_referensi')->where('nomor_referensi', $referensi)->oneArray();
      ...
      $booking_registrasi = $this->db('booking_registrasi')->where('tanggal_periksa', date('Y-m-d'))->where('no_rkm_medis', $no_rkm_medis)->oneArray();
      $reg_periksa = $this->db('reg_periksa')->where('tgl_registrasi', date('Y-m-d'))->where('no_rkm_medis', $no_rkm_medis)->oneArray();
      if($booking_registrasi['status'] == 'Terdaftar') {
        ...
        $cek_status_poli = $this->db('reg_periksa')->where('no_rkm_medis', $no_rkm_medis)->where('kd_poli', $booking_registrasi['kd_poli'])->count();
        $_POST['status_poli'] = 'Baru';
        if($cek_status_poli > 0) {
          $_POST['status_poli'] = 'Lama';
        }
        ...
        if($booking_registrasi['status'] == 'Belum') {
          $insert = $this->db('reg_periksa')
            ...
            if ($insert) {
                $this->db('booking_registrasi')->where('no_rkm_medis', $no_rkm_medis)->where('tanggal_periksa', date('Y-m-d'))->update('status', 'Terdaftar');
    ...
    public function getDaftarBPJS($nik)
    {
      ...
      $pasien = $this->db('pasien')->where('no_ktp', $nik)->oneArray();
      ...
      if($pasien) {
        ...
        if($this->db('pasien')->where('no_ktp', $nik)->oneArray()) {

...
class Admin extends AdminModule
{
    ...
    public function anyTaskID()
    {
      ...
      foreach ($query as $q) {
          ...
          $task1x = $this->db('referensi_mobilejkn_bpjs_taskid')->select('waktu')->where('no_rawat', $reg_periksa['no_rawat'])->where('taskid','1')->oneArray();
          $task2x = $this->db('referensi_mobilejkn_bpjs_taskid')->select('waktu')->where('no_rawat', $reg_periksa['no_rawat'])->where('taskid','2')->oneArray();
          $task1 = '';
          $task2 = '';
          ...
          if($taskid1['waktu'] == '') {
            $q['task1'] = $task1x['waktu'];
          }
          if($taskid2['waktu'] == '') {
            $q['task2'] = $task2x['waktu'];
          }
          if($taskid3['waktu'] == '') {

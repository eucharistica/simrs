...
class Admin extends AdminModule
{
    ...
    public function anySoap()
    {
      ...
      foreach ($rows as $row) {
        $row['nomor'] = $i++;
        $row['nama_petugas'] = $this->core->getPegawaiInfo('nama',$row['nik']);
        $row['departemen_petugas'] = $this->core->getDepartemenInfo($this->core->getPegawaiInfo('departemen',$row['nik']));
        $result[] = $row;
      }
    ...
    {
      $_POST['nik'] = $this->core->getUserInfo('username', null, true);
    ...
    {
      $data_resume['pemeriksaan_ralan'] = $this->db('pemeriksaan_ralan')->where('no_rawat', revertNoRawat($no_rawat))->desc('jam_rawat')->oneArray();

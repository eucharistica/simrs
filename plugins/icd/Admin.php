...
class Admin extends AdminModule
{
  ...
  public function postICD9RS()
  {

    if(isset($_POST["query"])){
      $output = '';
      $key = "%".$_POST["query"]."%";
      $rows = $this->db('icd9')->where('validcode', '1')->like('kode', $key)->orLike('deskripsi_panjang', $key)->asc('deskripsi_panjang')->limit(10)->toArray();
      $output = '';
      if(count($rows)){
        foreach ($rows as $row) {
  ...
  public function postICD10RS()
  {

    if(isset($_POST["query"])){
      $output = '';
      $key = "%".$_POST["query"]."%";
      $rows = $this->db('penyakit')->where('validcode', '1')->like('kd_penyakit', $key)->orLike('nm_penyakit', $key)->asc('nm_penyakit')->limit(10)->toArray();
      $output = '';
      if(count($rows)){
        foreach ($rows as $row) {

...
{
    private $_uploads = WEBAPPS_URL.'/berkasrawat/pages/upload';
    ...
    public function getManage()
    {
        ...
        $totalRecords = $this->db('pasien')
          ...
          ->leftJoin('sitb_pasien_norm','sitb_pasien_norm.no_rkm_medis=pasien.no_rkm_medis')
          ->toArray();
        ...
        $rows = $this->db('pasien')
          ->select('pasien.*')
          ->select('sitb_pasien_norm.no_sitb')
          ->leftJoin('sitb_pasien_norm','sitb_pasien_norm.no_rkm_medis=pasien.no_rkm_medis')
          ->desc('pasien.no_rkm_medis')
          ->offset(0)
          ->limit($perpage)
          ->toArray();
    ...
    public function anyDisplay()
    {
        ...
        $totalRecords = $this->db('pasien')->select('pasien.no_rkm_medis')->leftJoin('sitb_pasien_norm','sitb_pasien_norm.no_rkm_medis=pasien.no_rkm_medis')->toArray();
        $jumlah_data    = count($totalRecords);
  			$offset         = 10;
  			$jml_halaman    = ceil($jumlah_data / $offset);
        $halaman    = 1;
        ...
        if(isset($_POST['cari'])) {
          if(isset($_POST['halaman']) && $_POST['halaman'] !='') {
            ...
            $totalRecords = $this->db('pasien')
              ->select('pasien.no_rkm_medis')
              ->leftJoin('sitb_pasien_norm','sitb_pasien_norm.no_rkm_medis=pasien.no_rkm_medis')
              ->like('pasien.no_rkm_medis', '%'.$_POST['cari'].'%')
        ...
        $cek_vclaim = $this->db('mlite_modules')->where('dir', 'vclaim')->oneArray();
        $cek_pcare = $this->db('mlite_modules')->where('dir', 'pcare')->oneArray();
        $cek_sitb = $this->db('sitb_pasien_norm')->where('no_sitb', $row['no_rkm_medis'])->oneArray();
    ...
    public function postSaveBerkasDigital()
    {
      ...
      if(MULTI_APP) {
        ...
        $id = str_replace('/','_',$_POST['no_rawat']);
        $temp = explode(".", $_FILES["file"]["name"]);
        $imgName = time().$cntr++;
        $file = str_replace(' ','_',$_FILES['file']['name']);
        $lokasi_file = 'pages/upload/'.$id.'_'.$file;
    ...
    public function getRiwayatPerawatan($no_rkm_medis)
    {
      ...
      foreach ($reg_periksa as $row) {
        ...
        $row['pemeriksaan_ralan_rehab'] = $this->db('pemeriksaan_ralan_rehab')->where('no_rawat', $row['no_rawat'])->toArray();
        $row['uji_fungsi_kfr'] = $this->db('uji_fungsi_kfr')->where('no_rawat', $row['no_rawat'])->toArray();
        ...
        foreach ($rows_radiologi as $value) {
          ...
          $value['hasil_radiologi'] = $this->db('hasil_radiologi')
            ...
            foreach ($value['hasil_radiologi'] as &$hr) {
              if (!isset($hr['hasil']) || $hr['hasil'] === '') continue;
            
              $s = $hr['hasil'];
            
              // 1) NBSP: tangani dua kemungkinan (UTF-8 dan single-byte)
              $s = str_replace(["\xC2\xA0", "\xA0"], ' ', $s);
            
              // 2) Normalisasi newline
              $s = str_replace(["\r\n", "\r"], "\n", $s);
            
              // 3) Buang control char aneh (kecuali tab/newline)
              $s = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', '', $s);
            
              // 4) Paksa jadi UTF-8 valid (kalau ada mbstring)

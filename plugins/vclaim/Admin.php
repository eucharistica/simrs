...
class Admin extends AdminModule
{
  ...
  public function getCetakSEP($no_sep)
  {
    ...
    ->join('maping_poli_bpjs','maping_poli_bpjs.kd_poli_rs=poliklinik.kd_poli')   
    ->where('status','1')
    // ->where('kd_poli','!=','U0015')
    // ->where('kd_poli','!=','U0016')    
    ->where('kd_poli','!=','U0021')
    ...
    ->select('dokter.*')    
    ->join('maping_dokter_dpjpvclaim','maping_dokter_dpjpvclaim.kd_dokter=dokter.kd_dokter')
    ->where('status','1')
    ->asc('nm_dokter')
    ->toArray();
  ...
  public function getCetakSEP1($no_sep)
  {
    $settings = $this->settings('settings');
    $this->tpl->set('settings', $this->tpl->noParse_array(htmlspecialchars_array($settings)));
    $data_sep = $this->db('bridging_sep')->where('no_sep', $no_sep)->oneArray();
    ...
    ->asc('nm_dokter')
    ->toArray();
    echo $this->draw('cetak.sep1.html', ['data_sep' => $data_sep, 'poliklinik' => $poliklinik, 'dokter' => $dokter]);
    exit();
  }
  ...
  public function postSaveSEPInternal()
  {
    $date = date('Y-m-d');
    date_default_timezone_set('UTC');
    $tStamp = strval(time() - strtotime("1970-01-01 00:00:00"));
    ...
    if ($jenis_pelayanan == '2'){  
      if ($data['response']['noRujukan'] == "") {
        ...
        if ($data_rujukan['metaData']['code'] == 201) {
          ...
          $kodeppkperujuk = '09030100';
          $nmppkperujuk = 'PUSKESMAS MATRAMAN';
          $kddiagnosa = $_POST['kd_diagnosa'];
          $nmdiagnosa = $data['response']['diagnosa'];
          ...
          $no_telp = $data_rujukan['response']['rujukan']['peserta']['mr']['noTelepon'];
          $tglrujukan = $data_rujukan['response']['rujukan']['tglKunjungan'];
          $kodeppkperujuk = $data_rujukan['response']['rujukan']['provPerujuk']['kode'];
          $nmppkperujuk = $data_rujukan['response']['rujukan']['provPerujuk']['nama'];
          $kddiagnosa = $data_rujukan['response']['rujukan']['diagnosa']['kode'];
          $nmdiagnosa = $data_rujukan['response']['rujukan']['diagnosa']['nama'];
          $asalrujukan = $data_rujukan['response']['asalFaskes'];
        }
        ...
        if($data['response']['dpjp']['kdDPJP'] =='0')
          ...
          if ($data['metaData']['code'] == 200) {
            ...
            $insert = $this->db('bridging_sep')->save([
              ...
              'no_rujukan' => $data['response']['noRujukan'],
              'kdppkrujukan' => $kodeppkperujuk,
              'nmppkrujukan' => $nmppkperujuk,
              'kdppkpelayanan' => $this->settings->get('settings.ppk_bpjs'),
              'nmppkpelayanan' => $this->settings->get('settings.nama_instansi'),
              'jnspelayanan' => $jenis_pelayanan,
              'catatan' => $data['response']['catatan'],
              'diagawal' => $kddiagnosa,
              'nmdiagnosaawal' => $kddiagnosa.' - '. $nmdiagnosa,
              // 'kdpolitujuan' => $this->db('maping_poli_bpjs')->where('kd_poli_rs', $_POST['kd_poli'])->oneArray()['kd_poli_bpjs'],
      ...
      if (!empty($stringDecrypt)) {
        ...
        if ($data['metaData']['code'] == 200) {
          $insert = $this->db('bridging_sep')->save([
            ...
            'nmdiagnosaawal' => $_POST['kd_diagnosa'] . ' - ' . $data['response']['diagnosa'],
            'kdpolitujuan' => '',
            'nmpolitujuan' => '',
            'klsrawat' =>  $data['response']['klsRawat']['klsRawatHak'],
            'klsnaik' => $data['response']['klsRawat']['klsRawatNaik'] == null ? "" : $data['response']['klsRawat']['klsRawatNaik'],
  ...
  public function postSaveSEPInternal1()
  {
    $date = date('Y-m-d');
    date_default_timezone_set('UTC');
    $tStamp = strval(time() - strtotime("1970-01-01 00:00:00"));
      ...
      if ($data['response']['noRujukan'] == "") {
        ...
        if($data['response']['dpjp']['kdDPJP'] =='0')
          ...
          if ($insert) {
            $this->db('bpjs_prb')->save(['no_sep' => $data['response']['noSep'], 'prb' => $data_rujukan['response']['rujukan']['peserta']['informasi']['prolanisPRB']]);
            $this->notify('success', 'Simpan sukes');
            redirect(url([ADMIN, 'vclaim', 'cetaksep1', $_POST['no_sep'], convertNoRawat($_POST['no_rawat'])]));
          } else {
            $this->notify('failure', 'Simpan gagal');
            redirect(url([ADMIN, 'vclaim', 'cetaksep1', $_POST['no_sep'], convertNoRawat($_POST['no_rawat'])]));
          }
